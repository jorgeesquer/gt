...
func (f *File) AddDirective(directive string) error {
	if len(f.Imports) > 0 || len(f.Stms) > 0 || len(f.Comments) > 0 {
		return fmt.Errorf("a directive must appear at the beginning of the file: %s", directive)
	}
