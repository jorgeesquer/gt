/*
Package logdb implements a minimal time series database.
...
func New(path string, fs filesystem.FS) *DB {
	return &DB{
		Path:  path,
		mutex: &sync.RWMutex{},
		fs:    fs,
...
func (r *reader) open(t time.Time) (*os.File, error) {
	...
	if err != nil {
		...
		return nil, fmt.Errorf("logDB.open: error openning file %s: %v", path, err)
	}
	return f, nil
}
...
func (db *DB) save(t time.Time, table, data string, v ...interface{}) error {
	...
	if db.file == nil || db.writePath != fileName {
		...
		if err != nil {
			return fmt.Errorf("logDB: error creating dir %s: %v", dirName, err)
		}

		f, err := fs.OpenForAppend(fileName) //.OpenFile(fileName, os.O_APPEND|os.O_WRONLY|os.O_CREATE, 0644)
		if err != nil {
			return fmt.Errorf("logDB.Save: error openning file %s: %v", fileName, err)
		}
	...
	if _, err := fmt.Fprintf(db.file, "%s %s\n", t.Format(DATE_LAYOUT), data); err != nil {
		return fmt.Errorf("logDB: error writing data %v", err)
	}
