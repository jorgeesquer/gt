...
func exec_get(instr *Instruction, vm *VM) int {
	if err := vm.getFromObject(instr); err != nil {
		if vm.handle((err)) {
			return vm_continue
		} else {
			vm.Error = err
...
func exec_trw(instr *Instruction, vm *VM) int {
	...
	if v.Type == Object {
		if e, ok := v.ToObject().(Error); ok {
			// it's a re throw. Don't alter the stack trace
			err = e
		}
	}
...
func exec_new(instr *Instruction, vm *VM) int {
	// A class type, B retAddress, C argsAddress

	class := vm.get(instr.A).ToString()
	...
	i := newInstance(class, vm)

	v := NewObject(i)
	vm.set(instr.B, v)

	constructor := i.methodName("constructor")
	f, ok := vm.Program.Function(constructor)
	if ok {
...
func exec_cal(instr *Instruction, vm *VM) int {
	...
	return vm.call(instr.A, instr.B, args)
}
...
func exec_ret(instr *Instruction, vm *VM) int {
	...
	if !currentFrame.retValueSet {
		// A dest
		// check if we have to return a value
		if instr.A != Void {
			// save the return value in case a finally is executed after that
			// can overwrite it because finally's open a new block scope and reuse registers.
			currentFrame.retValueSet = true
			currentFrame.retValue = vm.get(instr.A)
		}
	}
	...
	var retValue Value
	if currentFrame.retValueSet {
		// if the ret value was stored to prevent finally code to overwrite it.
		retValue = currentFrame.retValue
	} else {
		if instr.A != Void {
	...
	if currentFrame.retValueSet {
		// cleanup also the protected retValues that are used to prevent being
		// overwritten in finally's
		currentFrame.retValueSet = false
		currentFrame.retValue = NullValue
	}
